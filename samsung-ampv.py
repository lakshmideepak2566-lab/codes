...
for env in envs:     
    ...
    env_vars_raw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SAMSUNG_AMPV_RAW_{env_for_deployment.upper()}"}
    ...
    env_vars_stdraw = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SAMSUNG_AMPV_STDRAW_{env_for_deployment.upper()}"}
    ...
    env_vars_rs = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SAMSUNG_AMPV_REDSHIFT_{env_for_deployment.upper()}"}
...
#    stdraw_task_dict["skipped_op"]>>ocv_task_dict["branch_op"]
    ...
    env_vars_lake = {**env_vars, "DATABRICKS_DEPLOYMENT_NAME" : f"CUMULUS_SAMSUNG_AMPV_STDRAW_DELTA_LAKE_{env_for_deployment.upper()}"}
    lake_task_list=PeptoDatabricksTaskList(dag=dag, env=env,source_id=source_id,process="dps_stdraw_delta_lake",super_package="laser_stdraw_pepto",package="dps_stdraw_delta_lake",previous_task_ids=[stdraw_task_dict["run_op"].task_id],env_vars=env_vars_lake)
